...
class GitManager:
    ...
    def clone_for_model(
        ...
        if dest.exists():
            # Ensure we're on the right branch if one was specified
            if branch:
        ...
        if branch:
            # Try cloning the existing branch directly (unshallow)
            try:
                self.repo.git.clone(
                ...
                # Branch doesn't exist on remote yet — fall through to default
                if dest.exists():
