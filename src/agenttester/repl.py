    ...
    try:
        while True:
            ...
            if raw == "/status":
                _background_tasks -= {t for t in _background_tasks if t.done()}
                waiting_names = {
                    q.model_name for q in question_registry.pending()
                }
                for name in models:
                    if name in waiting_names:
                        console.print(
                            f"  [yellow]⏸ {name}[/yellow]  waiting"
                        )
                    elif name in _busy_models:
                        console.print(
                            f"  [green]● {name}[/green]  running"
                        )
                    else:
            ...
            busy_in_target = {
                nm for nm in target_models if nm in _busy_models
            }
            if busy_in_target:
                console.print(
                ...
                target_models = {
                    nm: m for nm, m in target_models.items()
                    if nm not in busy_in_target
                }
