...
def _setup_git_and_tools(
    ...
    try:
        ...
        if git_mgr.has_commits():
            ...
            # Build a lookup of model_name → branch from the session
            _branch_lookup: dict[str, str] = {}
            for br in session_branches or []:
                # Branch format: agenttester/<model>/<slug>
                parts = br.split("/", 2)
                if len(parts) == 3:
                    _branch_lookup[parts[1]] = br

            for model in models.values():
                model_branch = _branch_lookup.get(
                    _sanitize_ref_component(model.name)
                )
...
async def run_repl(
    ...
    git_mgr = _setup_git_and_tools(
        workdir, models, session_name, pem_path, notify_url, question_registry,
        console, session_branches=session.branches,
    )
