...
def watch(
    ...
    if not session:
        ...
        event_dirs = [
            ...
            if d.is_dir() and (d / "events").is_dir()
        ]
        if not event_dirs:
            console.print("[red]No sessions with event logs found.[/red]")
            raise typer.Exit(1)
        session = max(
            event_dirs, key=lambda d: (d / "events").stat().st_mtime
        ).name
    ...
    if not model:
        ...
        if not events_path.is_dir():
            console.print(
                f"[red]No event logs for session {session!r}.[/red]"
            )
            raise typer.Exit(1)
        logs = sorted(events_path.glob("*.jsonl"))
        if not logs:
            console.print(
                f"[red]No model logs in session {session!r}.[/red]"
            )
            raise typer.Exit(1)
        if len(logs) == 1:
            model = logs[0].stem
            ...
            for n in names:
                console.print(f"  • {n}")
            console.print(
                "\n[yellow]Multiple models — specify one with "
                "-m/--model[/yellow]"
            )
            raise typer.Exit(1)
